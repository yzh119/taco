...
struct Cast {
  ...
  static __device__ __forceinline__ Type Encode(T val) { return static_cast<Type>(val); }
  static __device__ __forceinline__ T Decode(Type code) { return static_cast<T>(code); }
};

template <>
struct Cast<half> {
  typedef Code<sizeof(half)>::Type Type;
  static __device__ __forceinline__ Type Encode(half val) { return __half_as_ushort(val); }
  static __device__ __forceinline__ half Decode(Type code) { return __ushort_as_half(code); }
};

template <>
struct Cast<float> {
  typedef Code<sizeof(float)>::Type Type;
  static __device__ __forceinline__ Type Encode(float val) { return __float_as_uint(val); }
  static __device__ __forceinline__ float Decode(Type code) { return __uint_as_float(code); }
};

template <>
struct Cast<double> {
  typedef Code<sizeof(double)>::Type Type;
  static __device__ __forceinline__ Type Encode(double val) { return __double_as_longlong(val); }
  static __device__ __forceinline__ double Decode(Type code) { return __longlong_as_double(code); }
};

static __device__ __forceinline__ unsigned short int atomicCASshort(unsigned short int* address,
...
#define DEFINE_ATOMIC(NAME)                                                                 \
  ...
  __device__ __forceinline__ T Atomic##NAME(T* addr, T val) {                               \
    ...
    do {                                                                                    \
      assumed = old;                                                                        \
      old = atomicCAS(addr_as_ui, assumed, Cast<T>::Encode(OP(val, Cast<T>::Decode(old)))); \
    } while (assumed != old);                                                               \
 ...
 */
inline __device__ int64_t AtomicCAS(int64_t* const address, const int64_t compare,
                                    const int64_t val) {
  ...
  return atomicCAS(reinterpret_cast<Type*>(address), static_cast<Type>(compare),
 ...
 * \return The old value at the address.
 */
inline __device__ int32_t AtomicCAS(int32_t* const address, const int32_t compare,
                                    const int32_t val) {
  // match the type of "::atomicCAS", so ignore lint warning
...
__device__ __forceinline__ float AtomicAdd<float>(float* addr, float val) {
  ...
  do {
    assumed = old;
    old = atomicCAS(addr_as_ui, assumed, Cast<T>::Encode(Cast<T>::Decode(old) + val));
  } while (assumed != old);
  return Cast<T>::Decode(old);
#endif  // __CUDA_ARCH__
}
